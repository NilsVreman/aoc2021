...
fn read<R: Read>(io: R) -> Result<Vec<i64>, Error> {
    ...
    br.lines()
        .map(|line| line.and_then(|value| value.parse().map_err(|e| Error::new(ErrorKind::InvalidData, e))))
        .collect()
}

fn cmp_cons(vec: &Vec<i64>) -> i64 {
    return vec.iter().zip(vec.iter().skip(1)).fold(0, |acc, (&x, &y)| { 
...
fn main() -> Result<(), Error> {
    ...
    let i = cmp_cons(&vec);
    let vec3 = izip!(vec.iter(), vec.iter().skip(1), vec.iter().skip(2)).map(|(&x, &y, &z)| x+y+z).collect();
    let j = cmp_cons(&vec3);
